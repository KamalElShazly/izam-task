...
void main() {
  ...
  test('Test debounce emits only one item with multiple inputs incoming fast (< 500ms)', () async {
  ...
  test('Test debounce emits multiple items with multiple inputs incoming slow (> 500ms)', () async {
    bloc.sink.add(AddInventoryItem(Item(id: 1, name: 'Item 1', qty: 10, lastUpdated: DateTime.now())));
    // wait for debounce
    await Future.delayed(const Duration(milliseconds: 500));
    bloc.sink.add(AddInventoryItem(Item(id: 2, name: 'Item 2', qty: 20, lastUpdated: DateTime.now())));
